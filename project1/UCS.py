...
def insertAllyPieces(lines, lineNum):
    ...
    while lines[lineNum][0] == "[":
        ...
        lineNum += 1
    queue = []
    print("start = {}".format(coordToStrIntTuple(start)))
    visitSquare(start)
...
def search():
    ...
    while queue:
        hq.heapify(queue)
        cumulativeCost, posToSearch = hq.heappop(queue)
        ...
        for i in range (0, len(adjMat)):
            if not isVisited(adjMat[i]):
                ...
                hq.heappush(queue, (getPathCost(adjMat[i]) + cumulativeCost , adjMat[i]))
...
def run_UCS():
    ...
    if isComplete:
        while (crawl != -1):
            print("pathCost to be added = {}".format(getPathCost(crawl)))
