...
def initializeGameboard():
    ...
    return {('a', 2): ('Pawn', 'White'), ('b', 1): ('Pawn', 'White'), ('c', 1): ('Pawn', 'White'),
...
# def initializeGameboard(state):

#     state.gameboard = {('a', 1): ('Pawn', 'White'), ('b', 1): ('Pawn', 'White'), ('c', 1): ('Pawn', 'White'),
#          ('d', 1): ('Pawn', 'White'), ('e', 1): ('Pawn', 'White'), ('a', 3): ('Pawn', 'Black'),
#          ('b', 3): ('Pawn', 'Black'), ('c', 3): ('Pawn', 'Black'), ('d', 3): ('Pawn', 'Black'),
#          ('e', 3): ('Pawn', 'Black'), ('a', 0): ('Rook', 'White'), ('b', 0): ('Knight', 'White'),
#          ('c', 0): ('Bishop', 'White'), ('d', 0): ('Queen', 'White'), ('e', 0): ('King', 'White'),
...
#     else:
#         coeff = 1
#     if 0 <= row - (1 * coeff) < rows and 0 <= col < cols and not isFriendly(colour, state, rowColToCoord(row - (1 * coeff), col)):
...
# # both originalPos and newPos should be in (int, int) form
# def calcHeuristic(state, originalPos, newPos):
#     materialScore = {'King': 0, 'Pawn': 1, 'Bishop': 1, 'Knight': 1, 'Rook': 5, 'Queen': 9}
#     white = set()
#     black = set()
#     whiteScore = 0
#     blackScore = 0
#     originalPiece, originalColour = state.gameboard[coordToStrIntTuple(originalPos)]
    
#     enemyPos = genEnemyPos(state, originalPos)
#     # checking to see if an enemy piece is consumed
#     if coordToStrIntTuple(newPos) in enemyPos:
#         for coord in state.gameboard:
#             piece, colour = state.gameboard[coord]
#             if not coord == originalPos and not coord == newPos:
#                 threatList = genThreatList(piece, colour, state, strIntTupleToCoord(coord))
#                 if colour == "White":
#                     white.union(set(threatList))
#                     whiteScore = whiteScore + materialScore[piece]
...
#                     black.union(set(threatList))
#                     blackScore = blackScore + materialScore[piece]
#     white.union(set(genThreatList(originalPiece, originalColour, state, newPos)))
#     return whiteScore + len(white), blackScore + len(black)
...
#             threatList = genThreatList(piece, colour, stateToExpand, strIntTupleToCoord(coord))
#             for threat in threatList:
#                 whiteScore, blackScore = calcHeuristic(stateToExpand, strIntTupleToCoord(coord), threat)
#                 hq.heappush(placementQueue, (2 ** blackScore - 2 ** whiteScore, coord, coordToStrIntTuple(threat)))
#     # print(placementQueue)
...
# #     initialState = State({})
# #     initializeGameboard(initialState)
# #     # print(initialState)

# #     studentAgent(initialState.gameboard)